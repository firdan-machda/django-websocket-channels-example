...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.chat_message(
            {
                "type": "chat_message",
                "message": "available command is random, lorem, coinflip and ping",
                "owner": "server",
            }
        )
    ...
    async def receive(self, text_data):
        ...
        if message == "random":
            ...
            if answer:
                ...
                await self.chat_message(
                    {"message": "Flipped {}".format(result), "owner": "server"}
                )
                if result == answer:
                    await self.chat_message(
                        {"message": "Your guess is correct!", "owner": "server"}
                    )
                else:
                    await self.chat_message(
                        {"message": "Your guess is incorrect!", "owner": "server"}
                    )
            else:
                await self.chat_message(
                    {"message": "Guess coinflip: heads or tails?", "owner": "server"}
                )
                await self.prompt_action(
                    {
                        "choices": [
                            {"name": "Heads", "alias": "heads"},
                            {"name": "Tails", "alias": "tails"},
                        ]
                    }
                )
        else:
    ...
    async def chat_message(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "message", "message": message, "owner": event["owner"]}
            )
        )
    ...
    async def prompt_action(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "action", "choices": choices, "owner": "server"}
            )
        )
